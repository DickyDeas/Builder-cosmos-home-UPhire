/**
 * GDPR Right to Access – Export candidate data as JSON.
 * POST /api/export-candidate-data
 *
 * Body: { candidateId } or { email }
 * Returns candidate data for the authenticated user (must own the role/candidate).
 *
...
export async function handler(event) {
  ...
  let query = supabase.from("candidates").select("id, name, email, phone, status, created_at, role_id");
  ...
  const { data: role } = await supabase
    .from("roles")
    .select("id, title, profile_id, tenant_id")
